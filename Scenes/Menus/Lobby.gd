...
puppet func _override_max_team_amount(new_max_team_amount):
...
puppet func _override_table_ready_players(server_table_ready_players):
...
func _on_Lobby_visibility_changed():
	if visible:
		...
		#Add spectator team, and host, if they were not added from rejoining from a game.
