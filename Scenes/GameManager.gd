...
func _handle_dead_walker(walker):
	...
	if walker is load("res://Scenes/Entities/PlayerWalker.gd"):
		var camera = walker.get_node("Camera2D");
	...
	#Null the local player's walker if it was the killed walker.
	_local_players_walker = null if walker == _local_players_walker else _local_players_walker;
