...
func _physics_process(delta):
	...
	if !is_stunned():
		...
		if Input.is_action_pressed("walker_move") && get_local_mouse_position().length() > minimum_move_distance:
			...
			var rotation_speed = rotation_speed_sprint if _charge_state == CHARGING else rotation_speed_walk;
			...
			match _charge_state:
				...
				TURNING:
					...
					_charge_state = CHARGING if is_finished_rotating else TURNING;
